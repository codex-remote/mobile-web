...
状态：本地已实现；`429/5xx`、网络错误退避已测试，真实边缘和手机验收待完成。
...
- 429 优先遵循合法 `Retry-After`，最长等待 5 分钟；502/503、其他 5xx、网络断开与客户端超时从 750 ms 起做有上限指数退避，并加入 25% jitter。成功响应后重置退避，Cursor 在失败期间保持不变。
